...
Use after script approval to synthesize the canonical passage or article, measure real audio, and build one source-aligned `narration_timeline`.
...
3. Synthesize canonical section text in provider-safe chunks with the same voice and delivery profile. Persist both the Voice Design preview (when a voice is first created) and every narration WAV immediately. Stop on model-unavailable, billing, or free-tier exhaustion errors.
...
6. Join chunks only at recorded semantic pauses and keep word times monotonic on the aggregate timeline. In `article` mode, measured narration determines the video duration. In `phase1_verification` mode only, add end silence when under the fixed target; a bounded tempo adjustment still requires intelligibility review and words are never trimmed.
