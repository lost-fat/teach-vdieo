...
5. Author a provider-neutral `video_prompt_spec` for each scene: single-shot intent, subject motion, camera motion, timed action beats, continuity references, optional foreground parallax event, optional visual payoff, caption-safe area, and negative constraints. Motion should develop over time rather than animate a still frame in place.
