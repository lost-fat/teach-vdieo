...
Use this pipeline for a short English textbook passage or complete article that must remain verbatim while becoming a narrated, contextual video with bilingual, word-highlight captions.

## Prerequisites

Read `pipeline_defs/english-textbook.yaml`, the applicable fixed-verification or article-mode spec, every current stage director, `meta/reviewer`, and `meta/checkpoint-protocol`. Preflight the locked DashScope models, their current clip limits, free-tier-only policy, FFmpeg, and the selected composition runtime before any live generation.
...
4. Keep narration audio-first: no scene timing or article duration is final before the measured `narration_timeline` exists.
...
7. Before delivery, require `final_review.status == "pass"` and a measured MP4 within the selected duration policy. Require 9.90–10.10 seconds only for fixed verification mode.
